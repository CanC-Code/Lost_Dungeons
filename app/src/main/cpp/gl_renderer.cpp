...
namespace LostDungeons {
    ...
    // UI Tracking initialization
    float GLRenderer::compassX = 0.0f;
    float GLRenderer::compassY = 0.0f;
    float GLRenderer::compassRadius = 0.0f;
    float GLRenderer::menuBtnX = 0.0f;
    float GLRenderer::menuBtnY = 0.0f;
    float GLRenderer::menuBtnRadius = 0.0f;
    ...
    void GLRenderer::handleTap(float screenX, float screenY) {
        ...
        // 1. Menu Toggle Hit Detection (Always active overworld/menu)
        float mx = screenX - menuBtnX;
        float my = screenY - menuBtnY;
        if (std::sqrt(mx*mx + my*my) <= menuBtnRadius * 1.5f) {
            if (currentState == RenderState::OVERWORLD) {
                savedOverworldPos = cameraPos;
                savedOverworldYaw = yaw;
                savedOverworldPitch = pitch;
                currentState = RenderState::MENU;
            } else if (currentState == RenderState::MENU) {
                cameraPos = savedOverworldPos;
                yaw = savedOverworldYaw;
                pitch = savedOverworldPitch;
                currentState = RenderState::OVERWORLD;
            }
            return;
        }

        // 2. Compass Toggle Detection
        if (currentState == RenderState::OVERWORLD) {
            float dx = screenX - compassX;
            float dy = screenY - compassY;
            if (std::sqrt(dx*dx + dy*dy) <= compassRadius * 1.5f) {
                compassLockedToNorth = !compassLockedToNorth;
                return;
            }
        }

        // 3. In-Menu Interactions (Save / Inventory geometric bounds)
        if (currentState == RenderState::MENU) {
            float saveMinX = (width / 2.0f) - (width * 0.3f);
            float saveMaxX = (width / 2.0f) + (width * 0.3f);
            float saveMinY = (height * 0.4f) - (height * 0.05f);
            float saveMaxY = (height * 0.4f) + (height * 0.05f);
            
            if (screenX >= saveMinX && screenX <= saveMaxX && screenY >= saveMinY && screenY <= saveMaxY) {
                __android_log_print(ANDROID_LOG_INFO, "LostDungeonsUI", "Action: SAVE TRIGGERED");
                return;
            }

            float invMinX = (width / 2.0f) - (width * 0.3f);
            float invMaxX = (width / 2.0f) + (width * 0.3f);
            float invMinY = (height * 0.6f) - (height * 0.05f);
            float invMaxY = (height * 0.6f) + (height * 0.05f);

            if (screenX >= invMinX && screenX <= invMaxX && screenY >= invMinY && screenY <= invMaxY) {
                __android_log_print(ANDROID_LOG_INFO, "LostDungeonsUI", "Action: INVENTORY TRIGGERED");
                return;
            }
        }
    }
    ...
    void GLRenderer::updateTerrainMesh() {
        ...
        for (int z = -gridSize/2; z < gridSize/2; ++z) {
            for (int x = -gridSize/2; x < gridSize/2; ++x) {
                float fx = x * spacing; float fz = z * spacing;
                float worldX = camX + fx; float worldZ = camZ + fz;
                ...
                terrainVertices.insert(terrainVertices.end(), { fx, fy, fz, r, g, b, 1.0f, 0.0f, 1.0f, 0.0f });
            }
        }
    ...
    void GLRenderer::drawFrame() {
        ...
        // --- Recalculate Dynamic Layouts Every Frame ---
        float minDim = std::min(width, height);
        compassRadius = minDim * 0.08f;
        compassX = compassRadius + (minDim * 0.02f);
        compassY = compassX; 

        menuBtnRadius = minDim * 0.05f;
        menuBtnX = width - menuBtnRadius - (minDim * 0.02f);
        menuBtnY = compassY; 
        ...
        // --- 2D UI RENDERING (Proper Blending Included) ---
        glDisable(GL_DEPTH_TEST);
        glEnable(GL_BLEND);
        glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
        glUseProgram(uiShaderProgram);

        glm::mat4 orthoProj = glm::ortho(0.0f, (float)width, (float)height, 0.0f, -1.0f, 1.0f);

        if (currentState == RenderState::OVERWORLD || currentState == RenderState::MENU) {
            drawCompassHUD(engineTime, orthoProj); 
            drawMenuButton(orthoProj);
        }

        if (currentState == RenderState::MENU) {
            drawMenuOverlay(orthoProj);
        }
    ...
    void GLRenderer::drawCompassHUD(float engineTime, const glm::mat4& proj) {
        ...
        if (!initCompassMesh) {
            ...
            for (int i = 0; i < 16; ++i) {
                ...
                compVerts.insert(compVerts.end(), { cos(angle)*radius, sin(angle)*radius, r, g, b, 1.0f });
            }
            ...
            for (int i = 0; i <= ringSegments; ++i) {
                ...
                compVerts.insert(compVerts.end(), { cos(angle)*rInner, sin(angle)*rInner, 0.6f, 0.5f, 0.1f, 1.0f });
                compVerts.insert(compVerts.end(), { cos(angle)*rRing, sin(angle)*rRing, 0.9f, 0.8f, 0.2f, 1.0f });
            }

            for (int i = 0; i < ringSegments; ++i) {
                GLuint in1 = ringStartIdx + (i * 2);
                GLuint out1 = in1 + 1;
                GLuint in2 = in1 + 2;
                GLuint out2 = in1 + 3;
                compIndices.insert(compIndices.end(), { in1, out1, in2, out1, out2, in2 });
            }
            initCompassMesh = true;
        }

        glm::mat4 baseModel = glm::translate(glm::mat4(1.0f), glm::vec3(compassX, compassY, 0.0f));
        ...
        glm::mat4 needleModel = glm::rotate(baseModel, finalAngle, glm::vec3(0.0f, 0.0f, 1.0f));
        needleModel = glm::scale(needleModel, glm::vec3(compassRadius, compassRadius, 1.0f)); 
        glm::mat4 mvp = proj * needleModel;
        ...
        // Clock Orbit
        float timeOfDayAngle = (engineTime / 3000.0f) * 3.14159265f * 2.0f;
        float sunX = cos(timeOfDayAngle) * (compassRadius * 1.3f);
        float sunY = -sin(timeOfDayAngle) * (compassRadius * 1.3f); 

        glm::mat4 clockModel = glm::translate(baseModel, glm::vec3(sunX, sunY, 0.0f));
        clockModel = glm::scale(clockModel, glm::vec3(compassRadius * 0.2f, compassRadius * 0.2f, 1.0f));
        glm::mat4 clockMvp = proj * clockModel;
    ...
    void GLRenderer::drawMenuButton(const glm::mat4& proj) {
        auto drawUIRect = [&](float cx, float cy, float w, float h, glm::vec4 col) {
            float hw = w / 2.0f; float hh = h / 2.0f;
            GLfloat verts[] = {
                cx - hw, cy - hh, col.r, col.g, col.b, col.a,
                cx + hw, cy - hh, col.r, col.g, col.b, col.a,
                cx - hw, cy + hh, col.r, col.g, col.b, col.a,
                cx + hw, cy + hh, col.r, col.g, col.b, col.a
            };
            GLuint inds[] = {0, 1, 2, 1, 3, 2};
            glUniformMatrix4fv(uiMvpLoc, 1, GL_FALSE, glm::value_ptr(proj));
            glVertexAttribPointer(0, 2, GL_FLOAT, GL_FALSE, 6 * sizeof(GLfloat), verts);
            glVertexAttribPointer(1, 4, GL_FLOAT, GL_FALSE, 6 * sizeof(GLfloat), verts + 2);
            glEnableVertexAttribArray(0); glEnableVertexAttribArray(1);
            glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, inds);
        };
        
        float barW = menuBtnRadius * 1.6f;
        float barH = menuBtnRadius * 0.2f;
        glm::vec4 white(1.0f, 1.0f, 1.0f, 1.0f);
        
        drawUIRect(menuBtnX, menuBtnY - barH * 3.0f, barW, barH, white);
        drawUIRect(menuBtnX, menuBtnY,               barW, barH, white);
        drawUIRect(menuBtnX, menuBtnY + barH * 3.0f, barW, barH, white);
    }

    void GLRenderer::drawMenuOverlay(const glm::mat4& proj) {
        auto drawUIRect = [&](float cx, float cy, float w, float h, glm::vec4 col) {
            float hw = w / 2.0f; float hh = h / 2.0f;
            GLfloat verts[] = {
                cx - hw, cy - hh, col.r, col.g, col.b, col.a,
        ...
        // Darken the 3D scene
        drawUIRect(width / 2.0f, height / 2.0f, width, height, glm::vec4(0.0f, 0.0f, 0.0f, 0.75f));
        
        // Save Button (Green Box Placeholder)
        drawUIRect(width / 2.0f, height * 0.4f, width * 0.6f, height * 0.1f, glm::vec4(0.2f, 0.8f, 0.3f, 1.0f));
        
        // Inventory Button (Blue Box Placeholder)
        drawUIRect(width / 2.0f, height * 0.6f, width * 0.6f, height * 0.1f, glm::vec4(0.2f, 0.4f, 0.8f, 1.0f));
    }
}
