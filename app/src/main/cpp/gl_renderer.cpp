...
namespace LostDungeons {
    ...
    // -----------------------------------------------------------------------
    // Compass is drawn centered at (COMPASS_CX, COMPASS_CY) in screen pixels.
    // Needle radius * scale = 60 px → outer gold ring edge at ~78 px from center.
    // At (75, 75) the ring's topmost pixel is at y ≈ 75-78 = -3 (clipped by GL).
    // To guarantee it is fully visible: center = ring_outer_radius + margin.
    // ring_outer_radius = 60 (scale) * 1.0 (ringRadius in NDC) = 60 px
    // margin = 16 px → center = 76 px.  We use 80 for a clean look.
    // -----------------------------------------------------------------------
    static constexpr float COMPASS_CX = 80.0f;
    static constexpr float COMPASS_CY = 80.0f;
    // Hit radius for tap detection (slightly larger than the ring for comfort)
    ...
    // FIX: tap hit-region now uses the corrected COMPASS_CX/CY constants
    ...
    // FIX: added handling for RenderState::MENU (state == 2)
    void GLRenderer::setGameState(int state, const std::string& entityId) {
        ...
        // Cache overworld position whenever we leave it
        ...
        if (newState == RenderState::BATTLE) {
            ...
            // Menu overlay: keep camera frozen at current overworld position.
            // Camera is NOT moved; the C++ drawMenuOverlay() darkens the scene.
            // Restore overworld pos/yaw so the scene looks correct behind the dim.
            cameraPos = savedOverworldPos;
            yaw       = savedOverworldYaw;
            pitch     = savedOverworldPitch;
    ...
    void GLRenderer::updateInput(float moveX, float moveY, float lookX, float lookY) {
        std::lock_guard<std::mutex> lock(stateMutex);
        // Input is only processed in OVERWORLD; MENU and BATTLE ignore movement
    ...
    void GLRenderer::drawFrame() {
        ...
        glm::vec3 lookTarget =
            (currentState == RenderState::OVERWORLD || currentState == RenderState::MENU)
        ...
        // Draw the dim + Kotlin-driven menu UI signal when in MENU state
    ...
    // -----------------------------------------------------------------------
    // drawCompassHUD
    // Compass center is at (COMPASS_CX, COMPASS_CY) = (80, 80).
    // Needle scale = 60 px → outer gold ring edge ≈ 80 + 60 = 140 px from left/top.
    // This keeps the compass flush in the top-left corner with a clean 20 px margin
    // beyond the ring edge before any other UI element.
    // -----------------------------------------------------------------------
    void GLRenderer::drawCompassHUD(float engineTime) {
        ...
        // Y-axis inverted ortho (top=0, bottom=height) matching Android screen coords
        glm::mat4 projection = glm::ortho(0.0f, (float)width, (float)height, 0.0f, -1.0f, 1.0f);

        // FIX: compass center placed at (COMPASS_CX, COMPASS_CY) = (80, 80)
        ...
        // 2. Orbiting day/night clock diamond
        float timeOfDayAngle = (engineTime / 3000.0f) * 3.14159265f * 2.0f;
    ...
    void GLRenderer::drawMenuOverlay() {
        ...
        // 70% opacity black quad to dim the 3D scene behind the Kotlin menu UI
        GLfloat vertices[] = {
